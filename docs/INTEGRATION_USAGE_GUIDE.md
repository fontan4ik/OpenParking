...
For Miami curb-line correctness, run `npm run audit:parking-geometry:miami:refresh` after changing parking-space grouping or when the OSM road/building cache is stale; otherwise run `npm run audit:parking-geometry:miami`. The refresh command uses Overpass only to cache road centerlines and building polygons in `data/research/fetches/miami-osm-roads-buildings-cache.geojson`. The audit then checks generated curb rows locally against those roads/buildings plus official lot/garage polygons, and writes `data/research/miami-parking-geometry-quality-report.json`. Inspect `quality_thresholds`, `alignment_metrics`, `byStatus`, `byReason`, and `worst_examples`; the current baseline is `1825 accepted / 81 needs_field_review` from 1,906 published candidates. P95 `automatic_alignment_shift_meters` is 0 m (max 4 m), while P95 `official_point_to_generated_line_max_meters` is 3.3 m. Explicit `ParkMobile` zones are grouping boundaries, while unzoned legacy points remain compatible; a point gap over 18 meters starts a new row. Generated curb segments preserve the official point-row center/lateral offset and use a road matched against the whole row (start/midpoint/end) only for orientation and shape. Each consecutive vertex pair must be parallel to the matched road within the configured angle threshold, offset from the road centerline, near the road, and not crossing buildings or parking-area interiors; failures stay in review/reference state or are suppressed.

For Miami Beach zone payments, `payment_app_url` must use `https://www.paybyphone.com/park-in-miami-beach`. The detail panel displays and copies the exact `parkmobile_zone` before opening the official provider flow. Keep `payment_url` empty unless a stable checkout URL for that exact record is verified; keep garage reservation pages in `booking_url` only after exact facility/address/operator matching.
