...
DEV-55 добавляет design-only слой geometry quality перед implementation: `docs/ROAD_CURB_ENTRANCE_SNAPPING_HEURISTICS_DEV_55.md` и `data/research/road-curb-entrance-snapping-dev55.json` описывают, как привязывать street spaces/meters/signs к road/blockface, назначать `left/right/both` side, строить curb offset lines, резать сегменты на intersections/crosswalks/hydrants/driveways/loading/valet/no-parking conflicts, отделять parking-lot interiors, а OSM `parking_entrance` хранить как entrance/access evidence, не как отдельный parking offer. Ключевое правило: official/source confidence, geometry confidence и driver-facing `offer_confidence` должны быть разными фактами.
