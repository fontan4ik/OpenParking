...
| Главная карта | `apps/frontend/app/map/page.tsx`, `apps/frontend/components/ParkingMap.tsx` | Пользовательский map-first экран с MapLibre, OpenParking Atlas design contract, поиском, compact desktop context panel, native facility clusters до zoom 14, переключателями улицы/спутник и 3D-зданий, наклонным видом при активном маршруте, Yandex-inspired навигационной панелью `Откуда`/`Куда`, bottom sheets на mobile, auto-peek поведением панели при движении карты, отдельным режимом Data quality для review/conflict объектов и skeleton/error state при загрузке карты |
...
Важно: единые поля `source_url`, `api_url`, `payment_url`, `booking_url`, `evidence_url` уже протянуты через schema/loaders/API/UI contract. Для Miami Beach официальный provider flow хранится отдельно как `payment_app_url = https://www.paybyphone.com/park-in-miami-beach`: detail panel показывает конкретный `parkmobile_zone`, позволяет скопировать его и открывает официальный PayByPhone flow. Это не считается прямым `payment_url`, потому что пользователь ещё должен выбрать автомобиль, длительность и подтвердить транзакцию. Главный оставшийся product gap - стабильные per-record transactional links: в проверенных GeoJSON `payment_url` и `booking_url` почти везде отсутствуют, поэтому metrics честно отражают нулевое или низкое покрытие ссылками прямой оплаты/брони.

### 0.1.1 AI-помощник и маршрут

На карте есть компактная кнопка AI-помощника. Она принимает текст или локально распознанную через Whisper в браузере речь, подсказывает недорогие варианты только из текущего canonical/fallback parking data path и не выдает неизвестные цены за подтвержденные. Секрет `OPENROUTER_API_KEY` используется только в Next.js handler `/api/assistant`; handler фиксирован на `openrouter/free`, поэтому не выбирает платную модель как fallback. После первого нажатия на микрофон браузер скачивает Whisper-модель и выполняет транскрипцию локально; требуются сеть для первой загрузки и разрешение на микрофон. Выбор рекомендации открывает существующую карточку парковки, из которой используется текущий `/api/route` маршрут. При отказе geolocation маршрутный UI автоматически переводит пользователя к выбору стартовой точки на карте, а не блокирует сценарий.
...
- `miami_beach_parking_wpgmza.geojson` - adjacent Miami metro official-source layer. Он добавляет 74 public garage/lot records с координатами, spaces, hourly/event rates, ParkMobile zones и EV charging notes. Для записей с ParkMobile zone ParkingUSA протягивает `payment_provider = ParkMobile / PayByPhone`, canonical официальный Miami Beach PayByPhone URL `https://www.paybyphone.com/park-in-miami-beach` и `payment_note`; legacy `www2.paybyphone.com` нормализуется при загрузке, а `payment_url` остается пустым, пока нет точной per-record checkout-ссылки.
...
- `miami_beach_parking_arcgis_spaces.geojson` сохраняет 10,998 official street-space records как raw fixture по refresh 2026-07-15. Их нельзя показывать как отдельные прямоугольные полигоны или внутренние ряды парковки в curb-слое: frontend loader сначала исключает точки, попавшие внутрь parking lot/garage polygon, а оставшиеся street-side points группирует в road-following multi-vertex `LineString` curb rows. Явный `ParkMobile`/`parkmobile_zone` является границей группировки, поэтому соседние зоны вроде `88503` и `88526` не объединяются; точки без зоны сохраняют legacy-группировку. Соседние точки также разделяются при фактическом разрыве больше 18 метров. Если legacy rows приходят как Polygon/MultiPolygon, они нормализуются в линии по длинной оси. Parking lots остаются polygon zones, regulatory/residential rule polygons остаются вне normal curb layer. Для Miami добавлен детерминированный geometry QA: `apps/frontend/lib/parking-geometry-quality.ts` и `npm run audit:parking-geometry:miami` сверяют generated curb rows с OSM road centerlines, OSM buildings и official lot/garage polygons. Road matching оценивает начало, середину и конец ряда, поэтому короткая соседняя дорога у midpoint не может выиграть только из-за локальной близости. Официальные точки теперь являются главным источником центра и поперечного положения ряда: OSM задаёт направление/форму дороги, но не переносит ряд на фиксированные 4 м от road centerline. Minimum-offset guard применяется только к точкам, фактически попавшим на ось дороги. Generated curb segments публикуют `geometry_source_road_distance_meters`, `geometry_alignment_displacement_meters`, `geometry_lateral_position_source`, `official_point_fit_max_meters` и `geometry_accuracy_class=official_point_derived_road_oriented`; detail panel показывает эти метрики без заявления геодезической точности. Текущий QA baseline: `1825 accepted / 81 needs_field_review` из 1,906 опубликованных candidates; P95 автоматического сдвига равен 0 м (максимум 4 м), P95 максимального отклонения ряда от составляющих его официальных точек — 3.3 м. Каждая последовательная пара вершин должна быть параллельна дороге (в пределах порога), иметь offset от road centerline, находиться рядом с дорогой и не пересекать building/parking-area interior; непроходные линии остаются `needs_field_review`, пересечения с buildings/interiors подавляются. Паспорт ArcGIS layer 3 содержит старое описание/дату 2016 года, поэтому позиционная точность и свежесть конкретной записи остаются разными характеристиками; `last_edited_date` сохраняется в `raw_properties`.
...
- Facility point noise на дальнем и среднем масштабе снижается встроенной GeoJSON-кластеризацией MapLibre (`clusterMaxZoom=14`, `clusterRadius=56`). Кластеризация является только render-агрегацией: API, canonical records, dataset/map counters и отдельный `review-facilities` слой не объединяются. Клик по кластеру плавно переводит карту к expansion zoom, после чего отдельные парковки снова показывают semantic цвета цены/качества.
